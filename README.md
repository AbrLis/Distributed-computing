...
Далее представлены служебные API используемые самой программой для взаимодействия между оркестратором и агентами 

- API для запроса демоном задачи стоящей в очереди на вычисления

`GET запрос`
на адрес

`http://localhost:3000/get-task`

В ответ демон получит (если есть задача в очереди) ответ в виде JSON файла вида:
```json
{
  "id": "1707470348191170700",
  "expression": []массив операций необходимых для вычисления выражения,
}
```

Либо 404 если в очереди нет задачи.

----

- API для приема результата от вычислителя

`POST запрос`
на адрес

`http://localhost:3000/receive-result`

этот эндпоинт принимает JSON-файл вида
```json
{
  IDCalc string              `json:"id"`     - номер вычислителя который исполнял задача
  Result string              `json:"result"` - результат выполнения операции
  Status database.TaskStatus `json:"status"` - статус выполнения задачи
}
```

Этот эндпоинт обрабатывает данные и записывает результат в БД.
